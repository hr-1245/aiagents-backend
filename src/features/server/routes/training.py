...
from ...ai.models.models import (
    WebsiteCrawlRequest, DocumentUploadRequest, FAQTrainingRequest,
    TrainingResponse, CrawlStatus, MessageResponse, MessageRequest, 
    TrainingStatus, SummaryRequest, SupabaseFileTrainingRequest,
    SupabaseDocumentTrainingResponse, TrainingJobStatus,
    SimpleFileTrainingRequest, SimpleFAQTrainingRequest, 
    SimpleTrainingResponse, SimpleTrainingJobStatus
)
...
async def train_conversation(
    ...
    try:
        with API_LATENCY.labels('/conversation/train').time():
            ...
            logger.debug(f"TRAINING - Messages preview: {[{'role': msg.role, 'body_length': len(msg.body or '')} for msg in request.messages[:3]]}")
            ...
            logger.info(f"TRAINING - Cleaned {len(cleaned_messages)} messages for training")
            logger.debug(f"TRAINING - Sample cleaned message: {cleaned_messages[0] if cleaned_messages else 'None'}")
...
async def _crawl_website_background(crawl_id: str, request: WebsiteCrawlRequest):
    ...
    try:
        ...
        logger.info(f"Website crawl {crawl_id} completed: {documents_created} documents created")
    ...
    try:
        ...
        try:
            file_content, content_type = await supabase_service.download_file(
                bucket=request.supabaseBucket,
                file_path=request.fileId
            )
            logger.info(f"Downloaded file {request.fileName} ({len(file_content)} bytes)")
        except Exception as e:
...
async def _process_supabase_file_background(job_id: str, request: SupabaseFileTrainingRequest):
    ...
    try:
        ...
        logger.info(f"Supabase file training job {job_id} completed: {documents_created} vectors created")
...
async def train_file(
    ...
    try:
        ...
        try:
            file_content, content_type, file_info = await supabase_service.download_file_by_id(request.fileId)
            logger.info(f"Downloaded file {file_info['fileName']} ({len(file_content)} bytes)")
        except Exception as e:
...
async def train_faq(
    ...
    try:
        ...
        knowledgebase_id = request.knowledgebaseId or f"faq_{request.userId}_{int(datetime.now().timestamp())}"
...
async def _process_file_background(job_id: str, request: SimpleFileTrainingRequest):
    ...
    try:
        ...
        logger.info(f"File training job {job_id} completed: {documents_created} vectors created")
