...
if missing_vars:
    ...
    logger.error("Please create a .env file with the required variables or set them in your environment. Server cannot start.")
...
try:
    ...
    try:
        from features.server.security import create_cors_config, authenticate, check_rate_limit
    except ModuleNotFoundError:
        # Try relative import (when running from project root via main.py)
        from src.features.server.security import create_cors_config, authenticate, check_rate_limit
...
if os.environ.get("DEBUG_LOGS", "").lower() == "true":
    ...
    async def log_requests(request, call_next):
        ...
        if request.method in ["POST", "PUT", "PATCH"] and os.environ.get("LOG_REQUEST_BODY", "true").lower() == "true":
            try:
                body = await request.body()
                if body:
                    body_str = body.decode()
                    safe_body = safe_log_text(body_str, 500)  # First 500 chars only, Unicode-safe
    ...
    return {
        ...
        "components": {
            "api": {
                ...
                "details": "API router loaded successfully" if api_loaded else "API router failed to load"
            },
            "database": {
            ...
            "services": {
                "voyage": {
                    "status": "configured" if os.environ.get("VOYAGE_API_KEY") else "not configured"
                },
                "openai": {
                    "status": "configured" if os.environ.get("OPENAI_API_KEY") else "not configured"
                }
            }
        },
        "environment": {
            "python_version": sys.version.split()[0],
            "protobuf_implementation": os.environ.get("PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION", "unknown")
        }
    }
...
def run_server() -> None:
    ...
    try:
        ...
        if not os.environ.get("PROTOBUF_FIXED", ""):
            ...
            logger.warning("2. Set PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python in your environment")
        ...
        config = uvicorn.Config(
            ...
            access_log=True,
            timeout_keep_alive=int(os.environ.get("KEEP_ALIVE", "30")),  # Increased for better connection reuse
            workers=int(os.environ.get("WORKERS", "1")),  # Can be increased for production
            limit_concurrency=int(os.environ.get("LIMIT_CONCURRENCY", "1000")),  # Handle more concurrent requests
            limit_max_requests=int(os.environ.get("LIMIT_MAX_REQUESTS", "10000")),  # Higher request limit
            timeout_graceful_shutdown=int(os.environ.get("GRACEFUL_SHUTDOWN", "30")),  # Graceful shutdown
